...
51. **Eine Migration und ihr Rückbau schließen einander im selben Aufruf aus — entschieden, bevor der Runner die Datenbank berührt** (`G-077`). Der Gate-Runner geht die Gates der Reihe nach durch; standen `009` und `010` beide auf `true`, hätte derselbe Aufruf den Eigentumswechsel angewendet und ein paar Zeilen später zurückgebaut, beide Marker stehen lassen — und ein späteres korrektes `009` wäre als „already applied" übersprungen worden, mit `011` als einzigem Ausweg. Der Kommentar am Gate sagte „als eigener Aufruf"; erzwungen hat das nichts, und ein Kommentar erzwingt nichts (Leitplanke 7). Der Ausschluss steht jetzt vor dem ersten `psql`, fail-closed, und `test_bus_function_owner_rollback.py` prüft ihn samt Gegenprobe in beide Richtungen — auch die, dass ein Ausschluss **nach** der ersten Markerabfrage nicht zählt.

52. **Eine Migration, die eine vorgefundene Lage übernimmt, prüft die ganze Lage — nicht nur die Attribute, die sie selbst setzt** (`G-078`, `G-079`). `009` war darauf gebaut, eine vorhandene Rolle vorzufinden, und normalisierte, was sie kannte: Rollenattribute per `ALTER ROLE`, Objektrechte per `REVOKE ALL` vor jedem `GRANT`. Zwei Dinge bringt eine vorgefundene Lage außerdem mit, und beide blieben ungeprüft. **Mitgliedschaften:** `NOLOGIN` verhindert keinen `SET ROLE` — ein Mitglied von `workforce_owner` hätte dessen direkte Tabellenrechte außerhalb der zwölf Funktionen ausüben können. **Der bisherige Eigentümer:** `010` gibt das Eigentum an den *gelesenen* Schemaeigentümer zurück; das ist nur dann eine Wiederherstellung, wenn die Funktionen vorher genau ihm gehört haben — bei Drift hätte `009` sie trotzdem übernommen und `010` sie an die falsche Rolle gegeben. Beides wird jetzt **vor der ersten Änderung** geprüft und führt zum Abbruch; verändert wird nichts davon stillschweigend, denn eine Mitgliedschaft, die jemand angelegt hat, ist eine Entscheidung und kein Rest. Die Prüffrage lautet nicht „habe ich alles gesetzt, was ich setze", sondern „was kann diese Lage noch enthalten, das mein Ergebnis anders macht".

53. **Ein Claim ohne dauerhafte Wirkung wird zurückgegeben, und jeder Claim-Speicher hat eine Lease mit Versuchszähler** (`G-082`, `G-083`). Der Agent hielt den Claim, wenn das Budget *nach* dem Claim und *vor* dem Provideraufruf nicht mehr reichte: Die nächste Runde meldete die Nachricht dreißig Minuten lang als Dublette, und jede abgelaufene Lease kostete sie einen Versuch, den sie nie gemacht hatte — `G-013` mit umgekehrtem Vorzeichen, denn dort war etwas Dauerhaftes entstanden, hier nichts. Der Connector kannte gar keine Lease: Ein Update stand nach einem Absturz für immer auf `CLAIMED`, und die Wiederlieferung durch Telegram war eine „Dublette" — ein `/task` des CEO ohne Antwort, ohne Task und mit einer Auditzeile, die „schon erledigt" sagt. Beides ist dieselbe Frage wie `G-001`/`G-002`, nur einmal auf der Ausgangs- und einmal auf der Eingangsseite: Was dauerhaft ist, ist der Claim; was nicht dauerhaft ist, ist die Arbeit. Ein Claim wird atomar zurückgegeben, wenn nichts entstanden ist, und ein Claim, hinter dem niemand mehr arbeitet, verfällt nach einer Frist — innerhalb der Frist bleibt er eine Dublette, damit zwei Prozesse nicht dasselbe übernehmen.

54. **Zwei Datengrenzen beantworten dieselbe Frage gleich** (`G-084`). `G-029` hat den Betreff für die Modellgrenze als Inhalt eingestuft — Menschen schreiben die Anfrage hinein —, und die Telegram-Grenze schickte ihn unter derselben Policy `METADATA_ONLY` weiter, mit einem Kommentar, der „subject" unter Metadaten führte. Zwei Grenzen desselben Projekts, ein Name, zwei Bedeutungen. Wer eine Grenze verengt, sucht die andere; und die Feldmenge je Policy steht als Konstante da, gegen die ein Test prüft, nicht als Prosa (`G-054`).

55. **Der Wächter des Fensters misst selbst, und das Runbook ruft das Skript, das es gibt** (`G-085`, `G-086`). `nas_status.sh` — das Preflight, an dem jedes Gate hängt — nannte den API-Container beim abgeleiteten Namen (`G-042`) und zählte weder einen fehlgeschlagenen `docker exec` noch ein `psql: error` als Problem, weil der Status einer Pipeline der ihres letzten Befehls ist: genau die Falle, die dasselbe Skript acht Zeilen tiefer für `run_gate()` beschreibt und vermeidet. Und die Sollwerte, auf die das Runbook sich beruft — Kanal `DISABLED`, null Credentials, die exakte Migrationsmenge —, standen als Leseaufgabe da; ein fehlender Wert ist kein Abbruch, wenn niemand ihn vermisst (Regel 45). Jetzt vergleicht das Skript gegen `production_state.txt`. Das Runbook wiederum führte in Abschnitt 4 den Deploy-Einzeiler weiter aus, den `G-080` gerade aus dem Skript entfernt hatte — die zweite Abschrift einer Prozedur ist die, die im Fenster gelesen wird (`G-052`). Eine Prozedur, die als Skript existiert, wird aufgerufen, nicht abgeschrieben.
...
62. **Ein Test, der eine Secret-Invariante belegt, liest die Compose-Datei, nicht nur den Code** (`G-098`). `test_an_echo_only_setup_never_reads_the_model_key` war grün, während `compose.yaml` den Modellschlüssel in jeden Container hängte; der Code las ihn nicht, der Container hatte ihn trotzdem. Dieselbe Klasse wie `G-089` im Prototyp, und die Regel stand dort schon: Ein Container bekommt nur das Geheimnis, das er benutzt. Was fehlte, war der Test an der Stelle, die es entscheidet — die Mount-Liste. Und die Wahl trifft die Konfiguration, die ausgerollt wird, nicht der Inhalt von `secrets/`.

63. **Ein Betriebsskript wird gegen eine `ssh`-Attrappe in einem Wegwerf-Klon gemessen, sonst ist es Dokumentation** (`G-099`). Der Klon macht `git archive HEAD` echt und den Arbeitsbaum unerreichbar; die Attrappe zeichnet auf, was an die NAS gegangen wäre. Der erste Lauf des Tests hat den Sinn der Regel gleich mitgeliefert: Er prüfte das Skript aus `HEAD` und fiel durch, weil die Korrektur nur im Arbeitsbaum lag — `G-096` in Testform.

64. **Eine Wartezusage hat ein Ende, und die Meldung trägt den Tag** (`G-100`). „Wartet bis zum nächsten Tag" ohne Obergrenze ist ein unbegrenztes Warten, und eine Notiz, die auf ihrem Text dedupliziert, spricht bei konstantem Text genau einmal. Ein Zustand, der kein Versuch ist und keinen lädt, muss auf anderem Weg enden: hier nach `max_attempts` Tagen als `ABANDONED` mit Grund und sichtbarer Meldung (Invariante 15).
...
71. **Vor einem Deploy wird die Befunddatei neu gelesen, und der Freigabesatz nennt die offenen Befunde** (`G-114`). Gerds Parallelsitzung hatte am 2026-09-09 `G-112`/`G-113` mit rotem Gate eingetragen; Karl las am 2026-09-10 seinen Stand vom Vortag und rollte aus. Der Nachweis berief sich auf eine ältere Freigabe, die Vorlage an den CEO nannte die Befunde nicht, und der CEO sagte ja zu etwas, das er nicht wusste. Zwei Seiten schreiben in dieselbe Datei; wer daraus eine Freigabe ableitet, liest den Kopf der Datei in dem Moment, in dem er handelt, nicht in dem, in dem er die Vorlage schrieb. Und der Satz fürs Log trägt „offen: …" mit Nummern, damit ein Ja ein informiertes Ja ist.

72. **Die Wirkung kommt vor der Marke, und die Marke hängt am Zustand, nicht am Rückgabewert** (`G-112`). `set_setting` vor `record_inbound` machte aus einem Absturz einen Tag, der nie stattfand — Invariante 5 in der Klausel aus `G-082`, nur diesmal als Marke statt als Claim. Erst das Wiederholbare schreiben, dann die Marke. Und die Auditzeile dazu hängt an „gibt es sie schon", nicht an „habe ich den Datensatz gerade angelegt": Das `created` einer Runde ist in der nächsten Runde falsch.

73. **Was das Deploy-Skript hinterlässt, setzt und misst es selbst** (`G-115`, `G-116`). Die Rechte aus R-2 liefen von Hand im Wegwerf-Container und standen in keiner Datei; der nächste Deploy hätte sie stillschweigend überschrieben, und `verify` prüft die Datei, nie ihren Ordner. Und „deployed ⟨Commit⟩" war eine Behauptung über den Baum, keine Messung an ihm. Bauform-Zusage 3 in beide Richtungen: Der Rechtezustand wird im Skript gesetzt und per `stat` zurückgelesen, die ausgerollten Dateien werden per Prüfsumme gegen den Commit gehalten, beides mit Abbruch vor `up`.

74. **Ein Wächter unterscheidet „sauber" von „nicht gemessen", sonst ist er keiner** (Code-Review 2026-09-12, drei Befunde in einer Runde). Alle drei hatten dieselbe Form: eine leere Antwort galt als gutes Ergebnis. Die ACL-Prüfung ließ `""` durch, also auch ein fehlendes `synoacltool`; die Prüfung auf unerwartete Dateien wertete eine leere `find`-Ausgabe als „nichts Unerwartetes"; `_read_context` fing nur `FileNotFoundError` und ließ jeden anderen Lesefehler als Traceback enden. Abhilfe ist jedes Mal dieselbe: Die Antwort trägt eine Marke, die nur entstehen kann, wenn wirklich gemessen wurde (`ACL:0` statt `0`), ein leeres Ergebnis ist ein Abbruch, und jeder Fehlerpfad endet in einer stabilen Kennung. Das ist Regel 47 eine Ebene tiefer: Die Frage ist nicht nur, ob der Wächter rot werden **kann**, sondern ob er rot wird, wenn er gar nicht erst laufen konnte. **Und `set -eu` verschluckt die Meldung:** `x="$(befehl)"` bricht bei Fehlschlag ab, bevor `rc=$?` gelesen wird — der Exitcode gehört an dieselbe Zeile (`|| rc=$?`). **Nachtrag vom 2026-09-13, an der NAS gemessen:** Derselbe Wächter war danach immer noch blind, an zwei Stellen, die keine Attrappe zeigen kann. `synoacltool` liegt nicht im `PATH` einer nicht-interaktiven Sitzung — derselbe Fall wie `docker` und `/usr/local/bin/docker` —, und seine ACL-Zeilen beginnen mit einem **Tabulator**, den das Muster `^ *\[` nicht trifft: Gegen `/volume1/docker` mit acht Einträgen meldete er `0`. Ein Wächter über ein fremdes Werkzeug wird gegen einen Zustand gemessen, in dem er anschlagen **muss**, sonst ist die Gegenprobe nur die Attrappe, die man selbst gebaut hat.
