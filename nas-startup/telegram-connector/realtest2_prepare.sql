...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'TG_REALTEST2_MIGRATION_MISSING';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'TG_REALTEST2_CHANNEL_NOT_DISABLED';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'TG_REALTEST2_ACTIVE_CREDENTIAL_EXISTS';
    END IF;

    IF EXISTS (
        SELECT 1 FROM workforce.employees WHERE employee_id = 'CEO-TG-002'
    ) THEN
        RAISE EXCEPTION 'TG_REALTEST2_IDENTITY_ALREADY_EXISTS';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'TG_REALTEST2_TARGET_NOT_ACTIVE';
    END IF;

    IF EXISTS (
        SELECT 1 FROM workforce.bus_tasks WHERE task_id = 'ENG-TG-REALTEST-002'
    ) THEN
        RAISE EXCEPTION 'TG_REALTEST2_TASK_ALREADY_EXISTS';
    END IF;
END;
$$;

SELECT set_config('app.actor_id', 'SYSTEM-TG-REALTEST2', true);
SELECT set_config('app.request_id', 'TG-REALTEST2-PREPARE', true);
...
INSERT INTO workforce.employees (
    ...
    'CEO-TG-002',
    'Telegram CEO Connector – Acceptance 2',
    'SYSTEM_CONNECTOR',
...
INSERT INTO workforce.bus_route_allowlist (
    ...
    (
        'ROUTE-CEOTG002-AIENG001-MESSAGE',
        'START-UP',
        'CEO-TG-002',
        'AI-ENG-001',
        'MESSAGE',
