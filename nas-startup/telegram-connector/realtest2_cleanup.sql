...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'TG_REALTEST2_CLEANUP_CHANNEL_STATE_DENIED';
    END IF;

    IF EXISTS (
        SELECT 1 FROM workforce.employees
        WHERE employee_id = 'CEO-TG-002'
          AND source_ref <> 'DEC-026/ENG-007'
    ) THEN
        RAISE EXCEPTION 'TG_REALTEST2_CLEANUP_IDENTITY_SOURCE_MISMATCH';
    END IF;
END;
$$;

SELECT set_config('app.actor_id', 'SYSTEM-TG-REALTEST2', true);
SELECT set_config('app.request_id', 'TG-REALTEST2-CLEANUP', true);

UPDATE workforce.bus_credentials
SET credential_status = 'REVOKED',
    revoked_at = clock_timestamp(),
    revocation_reason = 'DEC-026 Telegram realtest completed or stopped'
WHERE credential_id = 'CRED-TG-CEO-REALTEST-002'
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'TG_REALTEST2_CLEANUP_ACTIVE_CREDENTIAL_REMAINS';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'TG_REALTEST2_CLEANUP_CHANNEL_NOT_DISABLED';
    END IF;
END;
$$;
