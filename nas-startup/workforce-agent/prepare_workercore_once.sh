...
: "${WC_SOURCE_REF:?set WC_SOURCE_REF}"
: "${WC_RUN_SUFFIX:?set WC_RUN_SUFFIX (credential ids must be unique per run)}"

secret_dir="${WC_SECRET_DIR:-/run/startup-workercore-secrets}"
mkdir -p "$secret_dir"

# Two tokens: the runtime under test, and the requester whose messages it
# answers. Generated here so the raw values never leave this host; only the
# hashes go into the database.
for who in agent karl; do
    token_file="$secret_dir/wc_token_$who"
...
agent_hash="$(hash_of "$secret_dir/wc_token_agent")"
karl_hash="$(hash_of "$secret_dir/wc_token_karl")"
...
for who in agent karl; do
    if ! chown 10001:10001 "$secret_dir/wc_token_$who" 2>/dev/null; then
        echo "BLOCKED: cannot hand wc_token_$who to UID 10001. Needs CAP_CHOWN." >&2
        exit 2
    fi
done
...
psql \
    ...
    -v source_ref="$WC_SOURCE_REF" \
    -v run_suffix="$WC_RUN_SUFFIX" \
    -f /opt/startup-workercore/workercore_prepare.sql

unset agent_hash karl_hash PGPASSWORD
